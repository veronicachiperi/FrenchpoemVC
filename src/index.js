function formatNutritionProgram(response) {
  ...
  lines.forEach((line, index) => {
    ...
    if (dayMatch) {
      ...
      days[currentDay] = dayContent.split(' - ').map(meal => meal.trim().replace(/(\d+)g\b/, '$1 grams'));
...
function generateProgram(event) {
  ...
  let context = "You are an expert in nutrition. Your mission is to generate 3 day nutrition programs with quantities in grams based on user weight in basic HTML. Make sure to follow the user weight. Do not include a title to the program. Sign the program with 'VC' inside a <strong> element at the end of the program and NOT at the beginning";
